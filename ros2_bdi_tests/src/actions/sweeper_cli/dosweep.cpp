...
class DoSweep : public plansys2::ActionExecutorClient
{
public:
  DoSweep()
  : plansys2::ActionExecutorClient("dosweep", 250ms)
  {
    progress_ = 0.0;
...
private:
  void do_work()
  {
    ...
    if (progress_ < 1.0) {
      progress_ += 0.0625;
      send_feedback(progress_, args[0] + "sweeping " + args[1] + " running");
    } else {
      finish(true, 1.0, args[0] + " sweeping " + args[1] + " completed");
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<DoSweep>();
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "dosweep"));
