...
class CarrierMovingBoxesSensor : public Sensor
{
    public:
        ...
        void performSensing()
        {
            ...
            // RCLCPP_INFO(this->get_logger(), "Current position x=%lf, y=%lf, z=%lf", current_position_.x, current_position_.y, current_position_.z);
            for(auto box : boxes_)
            {    
                if(boxes_positions_.count(box) > 0)
                {
                    // RCLCPP_INFO(this->get_logger(), "Box " + box + " position x=%lf, y=%lf, z=%lf", boxes_positions_[box].x, boxes_positions_[box].y, boxes_positions_[box].z);
                    // RCLCPP_INFO(this->get_logger(), "Box " + box + " position x=%d, y=%d, z=%d", boxWithinXBoundaries(box), boxWithinYBoundaries(box), boxWithinZBoundaries(box));
                    if(boxWithinXBoundaries(box) && boxWithinYBoundaries(box) && boxWithinZBoundaries(box))
                        moving_counter ++;
                }    
            }

            if(moving_counter != current_mb_.value)
            {    
                current_mb_.value = moving_counter;
                sense(current_mb_, ADD);//UPDATE current value of moving by agent boxes    
            }
        }
    ...
    private:
        ...
        */
        ...
        {
            return boxes_positions_[box].z > current_position_.z;
        }
